  ...
  <div class="col-md-12 text-center">
    <?php
     ...
     $count=$conn->prepare("SELECT * FROM following where user_id='".$_SESSION['user_id']."' and following_id='".$profile_id."' and status='1'");
        ...
        if($follow == ""){
        echo '<button  type="button" class="btn btn-primary form-control" style="color:white;width:900px;border:1px solid grey;" onclick=\'followfunc("' .$profile_id. '")\'>Follow</button>';

        }
        else
        {
        echo '<button  type="button" class="btn btn-primary form-control" style="color:white;width:900px;border:1px solid grey;" onclick=\'followfunc("' .$profile_id. '")\'>UnFollow</button>';
        }
      ...
      if($sql->rowCount()>0){
        foreach (($sql->fetchAll()) as $key => $row) {
          ...
          echo '<div class="col-md-4 text-center" style="padding: 0px;">
                <img src="../'.$row['blog_pic'].'" width="250" height="250" id="profile_blogs_image">
                </div>';
                $pic_count--;
          if($pic_count == '0')
          {
